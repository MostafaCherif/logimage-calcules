...
def create_window(nonogram: Nonogram):
    win = tk.Tk()
    MAX_HEIGHT = 600  # 500 for the nonogram, 100 for the constraints
    MAX_WIDTH = 600  # 500 for the nonogram, 100 for the constraints
    ...
    for vline_index in range(LOG_WIDTH):
        ...
        canvas.create_text(100 + (0.5 + vline_index) * 500 /
                           LOG_WIDTH, 15, text=list_to_vertical_str(nonogram.top_constraints[vline_index]), anchor="n")
    ...
    for hline_index in range(LOG_HEIGHT):
        ...
        canvas.create_text(15, 100 + (0.5 + hline_index) * 500 /
                           LOG_HEIGHT, text=list_to_horizontal_str(nonogram.left_constraints[hline_index]), anchor="w")
    canvas.pack()

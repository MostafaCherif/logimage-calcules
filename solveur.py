...
class NonogramSolver:
    ...
    def __init__(self, ROWS_VALUES, COLS_VALUES, savepath):
        ...
        self.board = [[0 for c in range(self.no_of_cols)]
        ...
        self.rows_possibilities = self.create_possibilities(
            ROWS_VALUES, self.no_of_cols)
        self.cols_possibilities = self.create_possibilities(
            COLS_VALUES, self.no_of_rows)

        while not self.solved:
            # step 2: Order indici by lowest
            self.lowest_rows = self.select_index_not_done(
                self.rows_possibilities, 1)
            self.lowest_cols = self.select_index_not_done(
                self.cols_possibilities, 0)
            self.lowest = sorted(
                self.lowest_rows + self.lowest_cols, key=lambda element: element[1])
                ...
                if not self.check_done(row_ind, ind1):
                    ...
                    for ind2, val in same_ind:
                        ...
                        if self.board[ri][ci] == 0:
                            ...
                            if row_ind:
                                self.cols_possibilities[ci] = self.remove_possibilities(
                                    self.cols_possibilities[ci], ri, val)
                            else:
                                self.rows_possibilities[ri] = self.remove_possibilities(
                                    self.rows_possibilities[ri], ci, val)
    ...
    def _create_possibilities(self, n_empty, groups, ones):
        ...
        for p in combinations(range(groups+n_empty), groups):
            ...
            for val in p:
                ...
                ones_idx += 1
            res_opt = [ones[val]+[-1] if val > -1 else [-1]
                       for val in selected]
    ...
    def save_board(self, increase_size=20):
        ...
        increased_board = np.zeros(
            np.array((self.no_of_rows, self.no_of_cols)) * increase_size)
        for j in range(self.no_of_rows):
            for k in range(self.no_of_cols):
                increased_board[j * increase_size: (j+1) * increase_size, k * increase_size: (
                    k+1) * increase_size] = self.board[j][k]
        plt.imsave(os.path.join(self.savepath,
                   f'{name}.jpeg'), increased_board, cmap='Greys', dpi=1000)
